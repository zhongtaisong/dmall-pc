...
export default class Store {
        ...
        if(Array.isArray(dataSource)) {
            dataSource.forEach(item => {
                ...
                if(Array.isArray(goods_picture)) {
                    item['goods_picture'] = goods_picture.filter(item02 => item02).map((item03, index) => {
                        return {
                            uid: Date.now() + index,
                            name: 'image.png',
                            status: 'done',
                ...
                if(Array.isArray(detail_picture)) {
                    item['detail_picture'] = detail_picture.filter(item02 => item02).map((item03, index) => {
                        return {
                            uid: Date.now() + index,
                            name: 'image.png',
                            status: 'done',
    ...
    adminBrandsSelectAllServiceFn = async () => {
        ...
        const { dataSource, } = result?.data?.content || {};
        runInAction(() => {
     ...
     * @param params 
     */
    adminGoodsUpdateStatusServiceFn = async (params: IAdminGoodsUpdateStatusService) => {
