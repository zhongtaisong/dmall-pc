...
export default class Store {
  ...
  selectAddressListServiceFn = async (callBack?: Function) => {
    ...
    runInAction(() => {
      ...
      if (Array.isArray(addressDataSource)) {
        ...
        currentAddress =
          addressDataSource?.find?.((item) => item?.isDefault) ||
          addressDataSource?.[0] ||
          {};
      }
  ...
  onValuesChange = (changedValues, values) => {
    this.currentAddress =
      this.addressDataSource.find(
        (item) => item?.id === changedValues?.addressId,
      ) || {};
  };
  ...
  shoppingCartSelectPidsServiceFn = async (goodsInfo: Array<IBuyGoodsInfo>) => {
    ...
    runInAction(() => {
      const goodsDataSource = result?.data?.context || [];
      let buyTotalNum = 0;
      let totalPrice = 0;
  ...
  orderAddServiceFn = async (params: IOrderAdd) => {
    ...
    if (result?.data?.code === SUCCESS_CODE) {
      history.push(`/views/order-details/${result?.data?.context}`);
    } else {
