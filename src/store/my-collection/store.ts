...
import { goodsCollectionSelectService } from './service';
// mobx数据
...
export default class Store {
  ...
  goodsCollectionSelectServiceFn = async () => {
    ...
    runInAction(() => {
      this.dataSource = result?.data?.context || [];
    });
  };
  ...
  commonCallBack = (pids: Array<number>) => {
    const selectedRowKeys = this.selectedRowKeys.filter(
      (item) => !pids.includes(item),
    );
