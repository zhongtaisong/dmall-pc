...
export default class Store {
  ...
  goodsDetailSelectServiceFn = async (id: number) => {
    ...
    runInAction(() => {
      this.goodsInfo = result?.data?.context || {};
    });
  };
  ...
  goodsEvaluateSelectServiceFn = async (params: ISelectGoodsDetail) => {
    ...
    runInAction(() => {
      this.evaluationList = result?.data?.context || [];
    });
  };
  ...
  goodsCollectionSelectPidsServiceFn = async (pids: Array<number>) => {
    ...
    runInAction(() => {
      this.isGoodsCollection = Boolean(
        Array.isArray(result?.data?.context) && result?.data?.context?.length,
      );
    });
  };
  ...
  shoppingCartAddServiceFn = async (
    ...
    const result = await store.commonStore.shoppingCartAddServiceFn(
      goodsInfo,
      url,
    );
    if (!result) return;
