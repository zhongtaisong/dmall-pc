...
import { 
    userLoginService, ILogin, userValidateService, IUserValidateService, 
    userUpdatePasswordService, IUserUpdatePasswordService,
} from './service';
...
export default class Store {
    ...
    userLoginServiceFn = async (params: ILogin) => {
        ...
        if(result?.data?.code === SUCCESS_CODE){
            ...
            const val = JSON.stringify(result?.data?.content || {});
     ...
     * @returns 
     */
    userValidateServiceFn = async (params: IUserValidateService, callBack?: Function) => {
        if(!params || !Object.keys(params).length) return;
        ...
        if(result?.data?.code === SUCCESS_CODE){
            callBack?.();
            runInAction(() => {
                this.temporaryToken = result?.data?.content?.temporaryToken || null;
                this.componentKey = 2;
            });
        }
    }
     ...
     * @returns 
     */
    userUpdatePasswordServiceFn = async (params: IUserUpdatePasswordService, callBack?: Function) => {
        if(!params || !Object.keys(params).length) return;
